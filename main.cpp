...
private:
    ...
    public:
        Board(Difficulty difficulty, const std::string& textureFile, Game& game) {
            ...
            loadFromFile(getFilenameForDifficulty(difficulty), game);
        }
    ...
    private:
        ...
        void loadFromFile(const std::string& filename, Game& game) {
            ...
            while (std::getline(file, line)) {
                ...
                switch (type) {
                case 1:
                    mSquares[start] = std::make_unique<Ladder>(start, end, game.ladderKnockSound);
                    break;
                case 2:
                    mSquares[start] = std::make_unique<Snake>(start, end, game.snakeHissSound);
                    break;
                case 3:
                    mSquares[start] = std::make_unique<Bonus>(start, game.bonusSound, game.bonusTexture);
                    break;
                default:
    ...
    void startGame(sf::RenderWindow& window, Difficulty difficulty, int numPlayers, sf::Music& music, const std::string& boardTexture) {
        ...
        Board board(difficulty, boardTexture, *this);
